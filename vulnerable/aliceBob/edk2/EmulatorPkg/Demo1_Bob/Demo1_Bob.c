...
_Array_ptr<UINTN> Storage :bounds(Address, Address + Size) = NULL;
...
Storage = _Assume_bounds_cast_M(UINTN* _Array, AllocatePool(Size),
                                 bounds(Address, Address + Size));

if ( Storage == NULL ) {
return EFI_INVALID_PARAMETER;
}

VOID* _Array Storage_ptr _Byte_count(Size) =
_Dynamic_bounds_cast_M(UINTN* _Array, Storage, byte_count(Size));

CopyMem(Storage_ptr, Address, Size);
