...
typedef struct {
  ...
  UINT16 e_res _Checked[4];   ///< Reserved words.
  UINT16    e_oemid;    ///< OEM identifier (for e_oeminfo).
  UINT16    e_oeminfo;  ///< OEM information; e_oemid specific.
  UINT16 e_res2 _Checked[10]; ///< Reserved words.
...
typedef struct {
  ...
  EFI_IMAGE_DATA_DIRECTORY DataDirectory _Checked[EFI_IMAGE_NUMBER_OF_DIRECTORY_ENTRIES];
...
///
typedef struct {
  UINT8 Name _Checked[EFI_IMAGE_SIZEOF_SHORT_NAME];
  union {
...
///
typedef struct {
  UINT8 Name _Checked[16];     ///< File member name - `/' terminated.
  UINT8 Date _Checked[12];     ///< File member date - decimal.
  UINT8 UserID _Checked[6];    ///< File member user id - decimal.
  UINT8 GroupID _Checked[6];   ///< File member group id - decimal.
  UINT8 Mode _Checked[8];      ///< File member mode - octal.
  UINT8 Size _Checked[10];     ///< File member size - decimal.
  UINT8 EndHeader _Checked[2]; ///< String to end header. (0x60 0x0A).
...
typedef struct {
  ...
  UINT32                  Name;
  _Ptr<EFI_IMAGE_THUNK_DATA> FirstThunk;
...
typedef struct {
  ...
  EFI_IMAGE_DATA_DIRECTORY DataDirectory _Checked[2];    ///< Only base relocation and debug directory.
} EFI_TE_IMAGE_HEADER;
