...
typedef _Ptr<CHAR16> EFI_STRING;
...
typedef struct {
  ...
  ///
  UINT8 GlyphCol1 _Checked[EFI_GLYPH_HEIGHT];
} EFI_NARROW_GLYPH;
...
typedef struct {
  ...
  /// on when normally displayed; those with zero are off.
  ///
  UINT8 GlyphCol2 _Checked[EFI_GLYPH_HEIGHT];
  ///
  /// Ensures that sizeof (EFI_WIDE_GLYPH) is twice the
  /// sizeof (EFI_NARROW_GLYPH). The contents of Pad must
  /// be zero.
  ///
  UINT8 Pad _Checked[3];
} EFI_WIDE_GLYPH;
...
typedef struct _EFI_HII_STRING_PACKAGE_HDR {
  ...
  CHAR16 LanguageWindow _Checked[16];
  EFI_STRING_ID             LanguageName;
  CHAR8 Language _Checked[1];
...
typedef struct _EFI_HII_IIBT_IMAGE_24BIT_BASE {
  UINT16               Width;
  UINT16               Height;
  EFI_HII_RGB_PIXEL Bitmap _Checked[1];
...
typedef struct _EFI_HII_IMAGE_PALETTE_INFO {
  UINT16               PaletteSize;
  EFI_HII_RGB_PIXEL PaletteValue _Checked[1];
...
typedef struct _EFI_HII_AIBT_OVERLAY_IMAGES_BLOCK {
  ...
  ///
  EFI_HII_ANIMATION_CELL AnimationCell _Checked[1];
