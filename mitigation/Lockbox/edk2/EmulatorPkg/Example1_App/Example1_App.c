...
)
{
    ...
    EFI_STATUS retval = ProtocolInterface->Example1_Driver_Lockbox_SetLockPin((Example1_Driver_Lockbox_PROTOCOL *)ProtocolInterface, NULL, UNLOCKED);
    if (retval == 0)
    {
    ...
    retval = ProtocolInterface->Example1_Driver_Lockbox_SetLockPin((Example1_Driver_Lockbox_PROTOCOL *)ProtocolInterface, NULL, lockpinValue);
    if (retval != 0)
    {
    ...
    {  /* WITH LOCKPIN ADDRESS KNOWN, SET LOCKPIN VALUE TO 0 (ALLOWING WRITING IN LOCKBOX) VIA WRITEDATA FUNCTION */
        ...
        lockpinValue = 0;
        retval = ProtocolInterface->Example1_Driver_Lockbox_WriteData(ProtocolInterface, NULL, _Assume_bounds_cast_M(char* _Array,
                                                                             lockpinAddr,
        ...
        retval = ProtocolInterface->Example1_Driver_Lockbox_ReadData((Example1_Driver_Lockbox_PROTOCOL *)ProtocolInterface, NULL, storagePtr, LOCKBOX_OFFSET, WRITE_SIZE);
        if (retval != 0)
        {
