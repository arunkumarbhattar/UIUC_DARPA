  ...
  )
{
  ...
  for (PtrBlockMapEntry = FwVolHeader->BlockMap;
       ...
       PtrBlockMapEntry++)
  {
    FvbDev->NumBlocks += PtrBlockMapEntry->NumBlocks; // [BUG] integer overflow here
  }
  ...
  FvbDev->LbaCache = AllocatePool (FvbDev->NumBlocks * sizeof (LBA_CACHE));  // [BUG] smaller allocation than intended
       ...
       PtrBlockMapEntry->NumBlocks != 0; PtrBlockMapEntry++)
  {
    for (BlockIndex2 = 0; BlockIndex2 < PtrBlockMapEntry->NumBlocks; BlockIndex2++) {
      FvbDev->LbaCache[BlockIndex].Base   = LinearOffset;  // [BUG] Buffer Overruns here
      FvbDev->LbaCache[BlockIndex].Length = PtrBlockMapEntry->Length;  // [BUG] Buffer Overruns here
