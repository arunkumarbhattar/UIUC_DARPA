...
Demo1_Alice_PROTOCOL * _Single AliceProtocol = NULL;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "bobkey's address is %p\n",
    __FUNCTION__, &bobKey));
  //
  ...
  Status = gBS->CreateEvent (
    ...
    (EFI_EVENT_NOTIFY)Demo1BobTimerHandler,           // NotifyFunction
    NULL,                           // NotifyContext
    &Demo1_Bob_PeriodicTimer        // Event
  );
  if (EFI_ERROR (Status)) {
...
Demo1BobDataProvider(
  ...
  IN VOID* _Array           *Dest _Byte_count(Size), //the bounds for this will be from Address to Size bytes
  IN UINTN                  Size
)
{
  ...
  VOID* _Array Storage _Byte_count(Size) = NULL;
  ...
  VOID* _Array Address_ptr _Byte_count(Size) =
        _Dynamic_bounds_cast_M(VOID* _Array, Address, byte_count(Size));

  CopyMem(Storage, Address_ptr, Size);
