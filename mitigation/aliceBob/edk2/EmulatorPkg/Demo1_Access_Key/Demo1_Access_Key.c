...
DEMO1_ACCESS_KEY *_Single masterKey = NULL;
...
/**
  ...
  @retval VOID
**/
_Checked void InsertFirst (
  DEMO1_ACCESS_KEY                    * _Single access_key
  )
{
  LINK * _Single link = (LINK * _Single)AllocatePool(sizeof(LINK)); // create a link
_Unchecked {
  ASSERT (link != NULL);
  }
  ...
  )
{
 ...
 //print access_key_lock
  ...
  // Create Master Key
  //
  masterKey = (DEMO1_ACCESS_KEY *_Single) AllocatePool(sizeof(DEMO1_ACCESS_KEY));
...
EFIAPI
Demo1GenerateAccessKey(
  IN Demo1_Access_Key_PROTOCOL        * _Single This,
  IN EFI_HANDLE                       Controller _Itype(VOID* _Single),
  IN BOOLEAN                          WriteAccess,
  IN OUT DEMO1_ACCESS_KEY             * _Single AccessKeyPtr // caller provided storage
  )
{
  EFI_STATUS Status = EFI_SUCCESS;
  UINTN header=0;
...
Demo1ValidateAccessKey (
  IN Demo1_Access_Key_PROTOCOL        * _Single This,
  IN EFI_HANDLE                       Controller _Itype(VOID* _Single),
  IN DEMO1_ACCESS_KEY                 * _Single AccessKeyPtr,
  IN BOOLEAN                          WriteAccess,
  IN OUT BOOLEAN                      * _Single Result
  )
{
  if ( Result == NULL ) {
    return EFI_INVALID_PARAMETER;
